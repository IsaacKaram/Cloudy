...
class WeekViewController: WeatherViewController {
    ...
    var weekViewModel : WeekViewViewModel? {
        didSet{
            updateView()
        }
    }
...
extension WeekViewController: UITableViewDataSource {
    ...
    func numberOfSections(in tableView: UITableView) -> Int {
        guard let weekViewModel = weekViewModel else { return 0 }
        return weekViewModel.numberOfSections
    }

    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        guard let weekViewModel = weekViewModel else { return 0 }
        return weekViewModel.numberOfDays
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if let weatherDayRepresentable = weekViewModel?.viewModel(for: indexPath.row){
