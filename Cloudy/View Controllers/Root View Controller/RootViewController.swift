    ...
    private func fetchWeatherData() {
        ...
        dataManager.weatherDataForLocation(latitude: latitude, longitude: longitude) { (response, error) in
            if let error = error {
                ...
                self.dayViewController.dayViewModel = DayViewViewModel(weatherData: response)

                // Configure Week View Controller
                self.weekViewController.weekViewModel = WeekViewViewModel(weatherData: response.dailyData)
            }
        }
    }
